...
//Rare exceptions to indicate a bug in the _nativeextension code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_nativeextension = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_nativeextension]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.nativeextension.module"

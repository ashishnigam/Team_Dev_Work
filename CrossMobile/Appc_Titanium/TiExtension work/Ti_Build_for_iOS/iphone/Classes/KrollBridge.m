...
NSString * NativeExtension$ModuleRequireFormat = @"(function(exports){"
...
@implementation NativeExtensionObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_nativeextension);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create NativeExtension global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "NativeExtension" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_nativeextension = [[NativeExtensionObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_nativeextension];
	
	NSString *_nativeextensionNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _nativeextensionNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_nativeextension valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:NativeExtension$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.nativeextension.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.nativeextension.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}

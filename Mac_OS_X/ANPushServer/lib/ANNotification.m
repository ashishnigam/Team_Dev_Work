...
@implementation ANNotification

#pragma mark - Init

- (id)initWithPayload:(NSString *)payload token:(NSString *)token identifier:(NSUInteger)identifier expirationDate:(NSDate *)date priority:(NSUInteger)priority
{
    self = [super init];
    if (self) {
...
- (NSString *)token
{
    return _tokenData ? [ANNotification hexFromData:_tokenData] : nil;
}
...
{
    switch (type) {
        case kANNotificationType0: return [self dataWithType0];
        case kANNotificationType1: return [self dataWithType1];
        case kANNotificationType2: return [self dataWithType2];
    }
    return nil;
}
...
+ (ANPusherServerResultCode)parseAPNSResponseData:(NSData *)data withIdentifier:(NSUInteger *)identifier
{
    ...
    if (command != 8) {
        return kANPusherResultUnexpectedResponseCommand;
    }
    uint8_t status = 0;
    ...
    switch (status) {
        case 0: return kANPusherResultAPNNoErrorsEncountered;
        case 1: return kANPusherResultAPNProcessingError;
        case 2: return kANPusherResultAPNMissingDeviceToken;
        case 3: return kANPusherResultAPNMissingTopic;
        case 4: return kANPusherResultAPNMissingPayload;
        case 5: return kANPusherResultAPNInvalidTokenSize;
        case 6: return kANPusherResultAPNInvalidTopicSize;
        case 7: return kANPusherResultAPNInvalidPayloadSize;
        case 8: return kANPusherResultAPNInvalidToken;
        case 10: return kANPusherResultAPNShutdown;
    }
    return kANPusherResultAPNUnknownReason;
}

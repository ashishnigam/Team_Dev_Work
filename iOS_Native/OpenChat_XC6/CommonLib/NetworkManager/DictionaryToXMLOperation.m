...
#import "DictionaryToXMLOperation.h"

@interface DictionaryToXMLOperation ()
{
...
@property (nonatomic, strong) NSDictionary *dictToParse;


//Converted XML string and Data Property
...
NSString *const kDictToXMLOprArrNodeTest = @"array";
NSString *const kDictToXMLOprDictNodeTest = @"dict";

NSString *const kDictToXMLOprTextNodeKey = @"text";

int const kDictToXMLOprProxyRange = 1000;

@implementation DictionaryToXMLOperation
...
-(void)main
{
    ...
    NSString *dictXMLString = [self xmlStringFromDict:self.dictToParse];
    ...
    if (![self isCancelled])
    {
        ...
        self.xmlString = dictXMLString;
        self.xmlData = [dictXMLString dataUsingEncoding:NSUTF8StringEncoding];
        
    }
    
    self.dictToParse = nil;
...
- (NSString *)xmlStringFromDict:(NSDictionary *)dict
{
    ...
    return [NSString stringWithString:xmlCompleteString];
}
...
#ifdef twoWayCompatibilityOpr
            if ([keyName isEqualToString:kDictToXMLOprTextNodeKey]) {
...
-(void)parseArrayComponent:(NSArray*)arrayObj forXMLNodeName:(NSString*)dictKeyName
{
    for (id obj in arrayObj) {
        if ([obj isKindOfClass:[NSArray class]]) {
            ...
            [self parseArrayComponent:obj forXMLNodeName:kDictToXMLOprArrNodeTest];
            ...
            [self parseDictComponent:obj forXMLNodeName:kDictToXMLOprDictNodeTest];
            [self writeEndElement:nil];
